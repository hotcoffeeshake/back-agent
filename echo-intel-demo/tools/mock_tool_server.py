...
class MockToolHandler(BaseHTTPRequestHandler):
    server_version = "EchoIntelMockToolGateway/0.1"
    ...
    def do_GET(self) -> None:
        ...
        try:
            if parts == ["health"]:
                self._send(HTTPStatus.OK, {"ok": True, "service": "echo-intel-mock-tool-gateway"})
                return
            if parts == ["scenarios"]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Echo Intel HTTP mock tool gateway.")
    ...
    print(f"Echo Intel mock tool gateway listening on http://{args.host}:{args.port}")
